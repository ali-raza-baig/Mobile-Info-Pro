## MobileInfoPro Task list

**Next Start from navbar and create an search bar for mobile etc.**



**Completed- Next start from collection controller**

**Home Page**
* Completed- Hero Section Search bar
* Choose Phone by category -pending
* Blogs section - removed


**Brand Page**
* Completed- search bar 
* completed- Latest SmartPhones
* Blog Section - removed

**Series Page**
* Completed- Search Bar 
* Completed- Latest Models
* Completed- Trending Smartphones under series

